...
public class info_Manage extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == jb1) {
            ...
            if(name.compareTo("123")==0&&password.compareTo("123")==0){
                if(jcb.getSelectedIndex()==1){
                    JOptionPane.showMessageDialog(null,"登录成功");
                    this.dispose();
                    seller_Manage seller_manage=new seller_Manage();
                    seller_manage.setVisible(true);
                }

            }else if(name.compareTo("1234")==0&&password.compareTo("1234")==0){
                if(jcb.getSelectedIndex()==0){
                    JOptionPane.showMessageDialog(null,"登录成功");
                    this.dispose();
                    custom_Manage custom_manage=new custom_Manage();
                    custom_manage.setVisible(true);
                }
            }
